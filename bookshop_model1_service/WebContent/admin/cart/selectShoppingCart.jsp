...
%>
	<div>
		<form action="<%= request.getContextPath() %>/admin/orders/insertOrderForm.jsp" method="post">
			<table>
				<thead>
					<tr>
					...
					%>
						<tbody>
							<tr>
								...
								<td><a href="<%= request.getContextPath() %>/admin/cart/deleteShoppingCartAction.jsp?shoppingcartNo=<%= bookJoinCartDTO.getShoppingCartDTO().getShoppingcartNo() %>">삭제</a></td>
							</tr>
						</tbody>
					<%
					}
...
<%
	...
	if(currentPage>1){
	%>
		<a href="<%= request.getContextPath() %>/admin/cart/selectShoppingCart.jsp?currentPage=<%= currentPage-1 %>">이전</a>
	<%
	}
	
	for(int i=0 ; i<lastPage ; i++){
	%>
		<a href="<%= request.getContextPath() %>/admin/cart/selectShoppingCart.jsp?currentPage=<%= i+1 %>"> <%= i+1 %> </a>
	<%
	}
	
	if(currentPage<lastPage){
	%>
		<a href="<%= request.getContextPath() %>/admin/cart/selectShoppingCart.jsp?currentPage=<%= currentPage+1 %>">다음</a>
	<%
	}
%>
