...
public class BookService {
	...
	public BookDTO selectBookCodePublisherService() {
		BookDTO bookDTO = new BookDTO();
		
		try {
			BookCodeDAO bookCodeDAO = new BookCodeDAO();
			//책 카테고리들을 전체검색하는 메서드 호출, 리턴값으로 카테고리들의 정보가 담긴 객체의 주소값을 저장한 배열객체의 주소값을 리턴한다.
			ArrayList<BookCodeDTO> bookCodeList = bookCodeDAO.selectBookCodeList();
			
			//배열객체의 주소값을 매개변수로 주소값을 저장하는 메서드 호출
			bookDTO.setBookCodeList(bookCodeList);
			
			if(null != bookCodeList) {
				BookPublisherDAO bookPublisherDAO = new BookPublisherDAO();
				//출판사들을 전체검색하는 메서드 호출, 리턴값으로 출판사들의 정보가 담긴 객체의 주소값을 저장한 배열객체의 주소값을 리턴한다.
				ArrayList<BookPublisherDTO> bookPublisherList = bookPublisherDAO.selectAllBookPublisher();
				
				//배열객체의 주소값을 매개변수로 주소값을 저장하는 메서드 호출
				bookDTO.setBookPublisherList(bookPublisherList);
		...
		System.out.println(bookDTO+"<--service메서드 처리 성공 여부");
		//리턴값은 배열객체들이 저장된 객체
		return bookDTO;
	}
